...
#define i2c_write(dev_addr, reg_addr, data_size,p_data) \
    __gyro_state->__impl.i2c_write(dev_addr,reg_addr,data_size,p_data)

#define i2c_read(dev_addr, reg_addr, data_size, p_data) \
    __gyro_state->__impl.i2c_read(dev_addr,reg_addr,data_size,p_data)

#define delay_ms __gyro_state->__impl.delay_ms
#define get_ms(p) do{ *p = __gyro_state->__impl.get_ms();}while(0)
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    if (!compass_result)
        result |= 0x04;
#else
// disabled to prevent init ret = -9
//        result |= 0x04;
#endif
restore:
#elif defined MPU6500
